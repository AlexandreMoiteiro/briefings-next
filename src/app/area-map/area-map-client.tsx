    ...
    const areas: CoordinateMapArea[] = savedAreas
      .map((area) => {
        if (area.id !== selectedAreaId) {
          return { id: area.id, name: area.name, points: area.points };
        }
    ...
    <div className="space-y-6">
      <section className="border-b border-zinc-200 pb-6">
        ...
        <p className="mt-4 max-w-3xl text-lg leading-8 text-zinc-600">
          Paste coordinates from NOTAM, GAMET or SIGMET messages, plot the affected areas and export the map to PDF.
        </p>
        <div className="mt-5 max-w-4xl rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm leading-6 text-amber-900">
          <strong>Accepted formats:</strong> compact DMS (384221N 0090058W), ICAO degrees/minutes (3842N 00900W or N3842 W00900), GAMET directional sectors such as S OF N3845 AND W OF W00815, DMS with symbols/spaces, decimal hemispheres and signed decimal pairs.
        </div>
      </section>
      ...
      <section className="grid gap-6 xl:grid-cols-[420px_1fr]">
        <aside className="space-y-5">
          <div className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm">
            ...
            <div className="mt-4 space-y-4">
              <label className="block space-y-2">
                <span className="text-xs font-semibold uppercase tracking-wide text-zinc-500">Select area</span>
                <select
              ...
              <div className="grid grid-cols-2 gap-3">
                <button type="button" onClick={saveNewArea} disabled={!canSave || busy} className="rounded-xl bg-zinc-950 px-4 py-2 text-sm font-semibold text-white transition hover:bg-zinc-800 disabled:bg-zinc-300">Save new</button>
                <button type="button" onClick={updateSelectedArea} disabled={!selectedAreaId || !canSave || busy} className="rounded-xl border border-zinc-200 px-4 py-2 text-sm font-semibold text-zinc-950 transition hover:bg-zinc-50 disabled:text-zinc-300">Update</button>
                <button type="button" onClick={deleteSelectedArea} disabled={!selectedAreaId || busy} className="rounded-xl border border-red-200 px-4 py-2 text-sm font-semibold text-red-700 transition hover:bg-red-50 disabled:text-zinc-300">Delete</button>
                <button type="button" onClick={newArea} className="rounded-xl border border-zinc-200 px-4 py-2 text-sm font-semibold text-zinc-950 transition hover:bg-zinc-50">Clear</button>
              </div>

              {areasStatus ? <p className="text-sm font-medium text-zinc-600">{areasStatus}</p> : null}
            </div>
          </div>

          <div className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm">
            <label className="block space-y-2">
              <span className="text-xs font-semibold uppercase tracking-wide text-zinc-500">Coordinates / GAMET area</span>
              <textarea
                value={input}
                ...
                placeholder={'SIGMET/GAMET examples:\nS OF N3845 AND W OF W00815\nN3842 W00900 - N3900 W00830\n384221N 0090058W - 384226N 0090052W'}
              />
            </label>

            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              <button type="button" onClick={copyGeoJson} disabled={!geoJson} className="rounded-xl border border-zinc-200 px-4 py-2 text-sm font-semibold text-zinc-950 transition hover:bg-zinc-50 disabled:text-zinc-300">Copy GeoJSON</button>
              <button type="button" onClick={exportPdf} disabled={!mapAreas.length || pdfBusy} className="rounded-xl bg-zinc-950 px-4 py-2 text-sm font-semibold text-white transition hover:bg-zinc-800 disabled:bg-zinc-300">
                {pdfBusy ? "Generating PDF..." : "Download PDF"}
              </button>
            </div>

            <label className="mt-3 block space-y-2">
              <span className="text-xs font-semibold uppercase tracking-wide text-zinc-500">PDF background</span>
              <select
                value={pdfSource}
                onChange={(event) => setPdfSource(event.target.value as AreaMapPdfSource)}
                className="w-full rounded-xl border border-zinc-200 bg-white px-3 py-2 text-sm"
              >
          ...
          <div className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm">
            <h2 className="text-lg font-semibold tracking-tight text-zinc-950">Points</h2>
            <p className="mt-1 text-sm text-zinc-500">{parsed.points.length} point(s) found.</p>

            {parsed.errors.length ? (
              <div className="mt-4 space-y-2">
                {parsed.errors.map((error) => (
                  <p key={error} className="rounded-xl border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700">{error}</p>
                ))}
              </div>
            ) : null}
              ...
              <div className="mt-4 overflow-hidden rounded-2xl border border-zinc-200">
                <table className="w-full text-left text-sm">
                  <thead className="bg-zinc-50 text-xs uppercase tracking-wide text-zinc-500">
                    <tr><th className="px-3 py-2">Point</th><th className="px-3 py-2">Lat</th><th className="px-3 py-2">Lon</th></tr>
                  </thead>
                  <tbody>
