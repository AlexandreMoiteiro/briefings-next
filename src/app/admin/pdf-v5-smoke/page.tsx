...
async function runSmokeTest() {
  ...
  try {
    ...
    if (performancePdf.getPageCount() !== 8) {
      throw new Error(`Expected 8 performance pages, got ${performancePdf.getPageCount()}.`);
    }
    ...
    const performanceSizes = performancePdf.getPages().map((page) => page.getSize());
    const a2Pages = performanceSizes.filter(
      (size) => Math.round(size.width) === 1191 && Math.round(size.height) === 1684
    ).length;
    if (a2Pages !== 4) {
      throw new Error(`Expected 4 A2 aerodrome pages, got ${a2Pages}.`);
    }
...
export default async function PdfV5SmokePage() {
