...
import { P2006TSourceMapperV9 } from "./p2006-source-mapper-v9";
import {
  STAGES,
  type Capture,
  type CaptureStore,
...
const MIGRATION_KEY = "briefings_p2006_guided_mapper_v9_symmetric_grid";
  ...
  for (const stage of STAGES) {
    if (stage.type === "performance") {
      for (const registration of P2006T_REGISTRATIONS) {
        for (const step of stage.steps) keys.add(mappingKey(stage, registration, step));
      }
    } else {
      for (const step of stage.steps) keys.add(mappingKey(stage, "CS-EAQ", step));
    }
  }
  return keys;
}
...
export function P2006TSourceMapperShellV9() {
  ...
  const totalExpected = useMemo(() => {
    ...
    const perAircraft = STAGES.filter((stage) => stage.type === "performance").reduce(
      (sum, stage) => sum + stage.steps.length,
      0
    );
  ...
  useEffect(() => {
    ...
    if (saved && !alreadyMigrated) {
      try {
        ...
        window.localStorage.setItem(STORAGE_KEY, JSON.stringify(cleaned.captures));
  ...
  async function importJson(file: File) {
    try {
      const flattened = flattenImportedPayload(JSON.parse(await file.text()) as unknown);
      const cleaned = cleanCaptures(flattened);
      window.localStorage.setItem(STORAGE_KEY, JSON.stringify(cleaned.captures));
      window.localStorage.setItem(MIGRATION_KEY, "1");
      setSummary({ kept: cleaned.kept, removed: cleaned.removed, source: file.name });
  ...
  return (
    <div className="space-y-4">
      <section className="rounded-3xl border border-emerald-200 bg-emerald-50 p-4 shadow-sm">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <h2 className="mt-1 text-lg font-semibold text-zinc-950">
              Keep shared form work and restart only the simplified table geometry
            </h2>
            <p className="mt-1 max-w-4xl text-sm leading-6 text-zinc-600">
              The importer preserves valid shared form rectangles and valid M&B geometry.
              Older table-line and four-cell calibrations are removed because each table now
              uses only two consecutive column lines, two consecutive row lines and one
              generated-grid confirmation.
            </p>
          </div>
        ...
        <div className="mt-3 flex flex-wrap items-center gap-3 text-xs text-zinc-600">
          <span className="rounded-full bg-white px-3 py-1 font-semibold">
            {totalExpected} total mapping items with symmetric auto-grid calibration
          </span>
          {summary ? (
            <span className="rounded-full bg-white px-3 py-1 font-semibold">
      ...
      {ready ? <P2006TSourceMapperV9 key={mapperKey} /> : null}
    </div>
  );
}
