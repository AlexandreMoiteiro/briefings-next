...
function registrationNumber(
  ...
  for (const token of encoded.split(",")) {
    ...
    const candidate = token.slice(0, separator).trim();
    const value = Number(token.slice(separator + 1));
    if (candidate === registration && Number.isFinite(value) && value > 0) {
...
function gridDimensions(
  ...
  const columns =
    registrationNumber(step.metadata?.columnCountByRegistration, registration) ??
    Number(step.metadata?.columnCount ?? 5);
  const rows =
    registrationNumber(step.metadata?.rowCountByRegistration, registration) ??
    Number(step.metadata?.rowCount ?? 22);
  return {
...
function gridFromCapture(
  ...
  const rowCenters = capture.points
    ...
    .map((point) => point.y);
  if (
    columnCenters.length !== columns ||
    rowCenters.length !== rows ||
    columnCenters.some(
...
function captureIsComplete(
  ...
  if (step.kind === "confirm") {
    if (!capture.confirmed) return false;
    if (!isAutoGridStep(step)) return true;
    const dimensions = gridDimensions(step, registration);
    return Boolean(
      gridFromCapture(capture, dimensions.columns, dimensions.rows)
    );
  }
  if (step.kind === "rect") return Boolean(capture.rect && capture.confirmed);
  if (step.kind === "point") {
    return capture.points.length === 1 && capture.confirmed;
  }
  if (step.kind === "points") {
    return (
      capture.points.length >= (step.requiredPoints ?? 1) && capture.confirmed
    );
  }
...
function stageIsComplete(
  ...
  return stage.steps.every((step) =>
    captureIsComplete(
      step,
      captures[mappingKey(stage, registration, step)],
      registration
    )
  );
}
...
function cruiseTextBlock(text: string, sourceId: string) {
  const match = sourceId.match(/^cruise-(\d+)$/);
  if (!match) return text;
  const marker = `Pressure Altitude: ${Number(match[1])} ft`;
  const start = text.indexOf(marker);
  if (start < 0) return text;
  const remaining = text.slice(start + marker.length);
  const next = remaining.search(/Pressure Altitude:\s*\d+\s*ft/);
  return next >= 0 ? remaining.slice(0, next) : remaining;
}

function countPublishedRows(
  text: string,
  ...
  const lines =
    source.auditFamily === "cruise"
      ? cruiseTextBlock(normalized, source.id).split(/\r?\n/)
      : normalized.split(/\r?\n/);
  ...
  if (source.auditFamily === "climb") {
    return lines.filter((line) => {
      ...
      return (clean.match(/-?\d+(?:\.\d+)?/g) ?? []).length >= 6;
    }).length;
  }
  ...
  if (source.auditFamily === "cruise") {
    return lines.filter((line) => {
      ...
      return (
        (clean.match(/-?\d+(?:\.\d+)?/g) ?? []).length >= expectedColumns
      );
    }).length;
  }
  return 0;
...
function FormBackground({ page, onReady }: { page: 1 | 2; onReady: () => void }) {
  const source =
    page === 1
      ? P2006T_FORM_PAGE_1_WEBP_BASE64
      : P2006T_FORM_PAGE_2_WEBP_BASE64;
  return (
...
function ColumnBoundaryHandles({
  ...
  const keyboardMove = (
    ...
    onMove(index, boundaries[index] + direction * (event.shiftKey ? 0.005 : 0.001));
  };
...
export function P2006TSourceMapper() {
  ...
  const [gridBoundaries, setGridBoundaries] =
    useState<GridBoundaryStore>({});
  ...
  useEffect(() => {
    try {
      ...
      if (savedBoundaries) {
        setGridBoundaries(
          JSON.parse(savedBoundaries) as GridBoundaryStore
        );
      }
    } catch {
    ...
    () =>
      STAGES.filter(
        (stage) =>
          ...
          stageIsComplete(stage, registration, captures)
      ).length,
    [captures, registration]
  );
  const totalPerformance = STAGES.filter(
  ...
  useEffect(() => {
    ...
    if (
      tab === "todo" &&
      completedPerformance === totalPerformance &&
      totalPerformance > 0
    ) {
      setTab("completed");
    }
  ...
  const dimensions = gridDimensions(step, registration);
  const sourceGridStep = source?.steps.find(isAutoGridStep);
  const sourceDimensions = sourceGridStep
    ? gridDimensions(sourceGridStep, registration)
    : dimensions;
  ...
  const currentColumnBoundaries = useMemo(() => {
    if (
      currentBoundaryRecord?.columnBoundaries.length ===
  ...
  const stageCompleteCount = stageCaptures.filter((entry) =>
    captureIsComplete(entry.step, entry.capture, registration)
  ).length;
  ...
  useEffect(() => {
    if (
      ...
      !isAutoGridStep(step) ||
      currentGrid ||
      (source?.grid?.regionRequired && !tableRegion)
    ) {
      return;
    }
    const timer = window.setTimeout(runAutoDetection, 60);
    return () => window.clearTimeout(timer);
    // currentKey changes whenever the selected aircraft, table or step changes.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [currentKey, imageReady, currentGrid, tableRegion]);
  ...
  useEffect(() => {
    ...
    setSourceCheck({
      state: "loading",
      parsedRows: 0,
      expectedRows: sourceDimensions.rows,
    ...
    void fetch(sourceAsset.text, { signal: controller.signal })
      ...
      .then((text) => {
        const parsedRows = countPublishedRows(
          text,
          source,
          sourceDimensions.columns
        );
        const expectedRows = sourceDimensions.rows;
        setSourceCheck({
          ...
          message:
            parsedRows >= expectedRows
              ? `${parsedRows} published rows recognised for ${registration}.`
              : `${parsedRows} of ${expectedRows} published rows recognised for ${registration}. Review the TXT extraction before approving this table.`,
        });
      })
      .catch(() => {
  ...
  }, [registration, source, sourceAsset, sourceDimensions.columns, sourceDimensions.rows]);
  ...
  function goNext() {
    if (stepIndex < stage.steps.length - 1) {
      ...
      const next =
        visibleStageIndexes[currentPosition + 1] ?? visibleStageIndexes[0];
  ...
  function runAutoDetection() {
    ...
    if (!image || !image.complete || !image.naturalWidth || detecting) return;
    if (source?.grid?.regionRequired && !tableRegion) {
      setSaveStatus("Select the upper or lower table region before detecting its grid.");
      return;
    }
    setDetecting(true);
    setColumnEditMode(false);
  ...
  function moveColumnBoundary(index: number, rawValue: number) {
    ...
    updated[index] = clamp(rawValue, previous, next);
    ...
    setGridBoundaries((current) => ({
      ...
      [currentKey]: {
        ...
        detectedColumnBoundaries:
          current[currentKey]?.detectedColumnBoundaries ?? starting,
        adjusted: true,
      },
    }));
    saveCapture(
  ...
  function saveAndAdvance(capture: Capture) {
    const confirmed = { ...capture, confirmed: true };
    if (roleOf(step) === "table-region") {
      const gridStep = stage.steps.find(isAutoGridStep);
      const gridKey = gridStep
        ? mappingKey(stage, registration, gridStep)
        : null;
      setCaptures((current) => {
        const next = { ...current, [currentKey]: confirmed };
        if (gridKey) delete next[gridKey];
        return next;
      });
      if (gridKey) {
        setGridBoundaries((current) => {
      ...
      saveCapture(currentKey, confirmed);
    }
    resetInteraction();
    window.setTimeout(goNext, 100);
  }
  ...
  function handlePointerUp(event: ReactPointerEvent<HTMLDivElement>) {
    ...
    if (manualGridBoxMode) {
      saveDetectedGrid(
        gridFromManualBox(rect, dimensions.columns, dimensions.rows)
      );
      resetInteraction();
      return;
    }
        ...
        pdfRect:
          pdfGeometry && capture.rect
            ? {
                ...
                y:
                  (1 - capture.rect.y - capture.rect.height) *
                  A4_HEIGHT_PT,
      ...
      <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        <div>
          ...
          <h2 className="mt-1 text-2xl font-semibold tracking-tight text-zinc-950">
            Aircraft-specific grids and adjustable vertical lines
          </h2>
          <p className="mt-2 max-w-4xl text-sm leading-6 text-zinc-600">
            Cruise grids follow each AFM revision automatically. CS-EAQ keeps
            L/h and US gal/h columns; CS-EBX and D-GSEV use the narrower L/h-only
            tables.
          </p>
        </div>
      ...
      {visibleStageIndexes.length ? (
        <nav className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {visibleStageIndexes.map(({ stage: candidate, index }) => {
            const complete = stageIsComplete(
              candidate,
              registration,
              captures
            );
            ...
            return (
              ...
              >
                ...
                <span className="mt-1 block text-xs opacity-70">
                  {complete
                    ? "Completed"
                    : `${done}/${candidate.steps.length} complete`}
                </span>
              </button>
            );
          })}
        ...
        <div className="rounded-3xl border border-zinc-200 bg-white p-4">
              ...
              <div className="mt-4 space-y-3">
                <div className="flex flex-wrap gap-2 text-xs font-semibold">
                  <span className="rounded-full bg-white px-3 py-1.5 text-zinc-700">
                    {dimensions.columns} columns × {dimensions.rows} rows · {registration}
                  </span>
                  <span className="rounded-full bg-white px-3 py-1.5 text-zinc-700">
                    {currentGrid
                ...
                {columnEditMode ? (
                  <p className="rounded-xl border border-sky-200 bg-white p-3 text-xs leading-5 text-sky-900">
                    Drag each blue vertical handle independently, including the
                    left and right edges. Arrow keys provide fine adjustment;
                  ...
                  <>
                    ...
                    step.lineMode === "polyline" ? (
                      <button
                        type="button"
                        disabled={
                          !currentCapture || currentCapture.points.length < 2
                        }
          ...
          <div className="mb-3 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-zinc-200 bg-zinc-50 px-3 py-2">
            ...
            <label className="flex items-center gap-2 text-xs font-semibold text-zinc-600">
              <input
                type="checkbox"
                checked={showCompletedGeometry}
                onChange={(event) =>
                  setShowCompletedGeometry(event.target.checked)
                }
              />
          ...
          <div className="max-h-[78vh] overflow-auto rounded-2xl border border-zinc-300 bg-zinc-100 p-2">
            <div
              ...
              style={{
                ...
                touchAction:
                  captureMode || manualGridBoxMode || columnEditMode
                    ? "none"
                    : "auto",
              }}
        ...
        <aside className="space-y-4">
          <section className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm">
            ...
            <p className="mt-2 text-2xl font-semibold">
              {stageCompleteCount}/{stage.steps.length}
            </p>
            <select
              value={stepIndex}
              onChange={(event) => {
          ...
          {sourceAsset ? (
            <section className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm">
              ...
              <p className="mt-1 text-sm text-zinc-600">
                PDF {sourceAsset.pdfPage} · AFM {sourceAsset.printedPage}
              </p>
              {source?.grid ? (
                <div
          ...
          {isAutoGridStep(step) && currentGrid ? (
            <section className="rounded-3xl border border-sky-200 bg-white p-5 shadow-sm">
              ...
              <p className="mt-2 text-sm leading-6 text-zinc-600">
                {currentColumnBoundaries.length} vertical boundaries define{" "}
                {dimensions.columns} independently sized columns for {registration}.
              </p>
                ...
                {currentColumnBoundaries
                  ...
                  .map((left, index) => (
                    <div
                      ...
                      className="rounded-xl bg-zinc-50 p-2 text-zinc-700"
                    >
                      <span className="font-semibold">Column {index + 1}</span>
                      <span className="ml-2 font-mono">
                        {(
                          (currentColumnBoundaries[index + 1] - left) *
                          100
                        ).toFixed(2)}
                        %
                      </span>
