...
const MIGRATION_KEY = "briefings_p2006_auto_grid_shell_v10";
...
export function P2006TSourceMapperShellV10() {
      ...
      <section className="rounded-3xl border border-emerald-200 bg-emerald-50 p-4 shadow-sm">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-emerald-700">
              Resume existing form work
            </p>
            <h2 className="mt-1 text-lg font-semibold text-zinc-950">
              Keep manual form coordinates; replace old table geometry automatically
            </h2>
            <p className="mt-1 max-w-4xl text-sm leading-6 text-zinc-600">
              Importing an older JSON preserves valid form rectangles and M&B geometry.
              Obsolete performance row/column captures are discarded because every table
              now receives an automatically detected 5 × 22 visual grid.
            </p>
          </div>
        ...
        <div className="mt-3 flex flex-wrap items-center gap-3 text-xs text-zinc-600">
          <span className="rounded-full bg-white px-3 py-1 font-semibold">
            {totalExpected} mapping items after automatic grid detection
          </span>
          {summary ? (
            <span className="rounded-full bg-white px-3 py-1 font-semibold">
