...
function migrateCaptures(input: CaptureStore): CaptureStore {
  ...
  for (const [key, capture] of Object.entries(next)) {
    if (!key.endsWith(":row-seed")) continue;

    if (capture.points.length === 4) {
      const firstY = average(capture.points[0].y, capture.points[1].y);
      const secondY = average(capture.points[2].y, capture.points[3].y);
      const x = average(...capture.points.map((point) => point.x));
      const ordered = [firstY, secondY].sort((a, b) => a - b);
      next[key] = {
        ...capture,
        points: [
          { x, y: ordered[0] },
    ...
    } else if (capture.points.length < 3) {
      next[key] = { ...capture, confirmed: false };
    }
...
function orderedRowSeeds(points: Point[]) {
  return points
    .slice(0, 3)
    .map((point) => point.y)
    .sort((a, b) => a - b);
...
function lockedRows(points: Point[], columns: Segment[]): Segment[] {
  const seedY = orderedRowSeeds(points);
  if (seedY.length !== 3 || columns.length !== 5) return [];

  const withinPair = seedY[1] - seedY[0];
  const betweenAltitudes = seedY[2] - seedY[1];
  if (withinPair <= 0 || betweenAltitudes <= 0) return [];

  const yValues = [...seedY];
  for (let index = 3; index < 22; index += 1) {
    const gap = index % 2 === 1 ? withinPair : betweenAltitudes;
    yValues.push(yValues[index - 1] + gap);
  }
...
function deriveGrid(columnCapture?: Capture, rowCapture?: Capture): DerivedGrid | null {
  ...
  const rows = lockedRows(rowCapture.points, columns);
  const rowSeed = orderedRowSeeds(rowCapture.points);
  if (columns.length !== 5 || rows.length !== 22 || rowSeed.length !== 3) {
    return null;
  }
  ...
  return {
    ...
    intersections,
    spacing: {
      withinPair: rowSeed[1] - rowSeed[0],
      betweenAltitudes: rowSeed[2] - rowSeed[1],
    },
  };
}
...
function activeInstruction(step: GuidedStep, capture?: Capture) {
  if (step.kind === "rect") {
    return step.metadata?.role === "calculation-notes-rectangle"
      ? "Drag the white area. Release to save it and continue."
...
function CaptureOverlay({
  ...
  if (capture.kind === "rect" && capture.rect) {
    const isCalculationBox = role === "calculation-notes-rectangle";
    return (
      <div
        ...
        style={{
          ...
          borderColor: stroke,
          background: isCalculationBox ? "rgba(255,255,255,0.96)" : "rgba(5,150,105,0.08)",
          boxShadow: isCalculationBox ? "0 0 0 1px rgba(0,0,0,0.08)" : undefined,
        }}
      >
        {isCalculationBox ? (
  ...
  if (role === "regular-column-seed") {
    segments = lockedColumns(capture.points).slice(0, 2);
  } else if (role === "regular-row-seed" && columns.length === 5) {
    const leftX = columns[0].start.x;
    const rightX = columns[columns.length - 1].start.x;
    segments = orderedRowSeeds(capture.points).map((y) => ({
      start: { x: leftX, y },
      end: { x: rightX, y },
    }));
  }
...
function PdfPageImage({ page, onReady }: { page: 1 | 2; onReady: () => void }) {
  ...
  useEffect(() => {
    ...
    async function render() {
      ...
      try {
        ...
        const viewport = sourcePage.getViewport({ scale: 2.1 });
...
export function P2006TSourceMapper() {
  ...
  const gridConfirmation =
    stage.type === "performance"
      ...
      : undefined;
  const columns = columnSeed ? lockedColumns(columnSeed.points) : [];
  const derivedGrid =
    stage.type === "performance" ? deriveGrid(columnSeed, rowSeed) : null;
  ...
  const stageConfirmed = stageCaptures.filter(
    (entry) => captureIsComplete(entry.step, entry.capture)
  ).length;
  ...
  function exportMap() {
      ...
      STAGES.filter((candidate) => candidate.type === "performance").flatMap(
        (candidate) =>
          P2006T_REGISTRATIONS.map((candidateRegistration) => {
            ...
            const grid = deriveGrid(candidateColumns, candidateRows);
            const calculationStep = candidate.steps.find(
              (candidateStep) => candidateStep.id === "calculation-notes-rectangle"
            );
            const calculationCapture = calculationStep
              ? captures[mappingKey(candidate, candidateRegistration, calculationStep)]
              : undefined;
            ...
            return [
              ...
              grid
                ? {
                    ...
                    calculationNotesRectangle: calculationCapture?.rect ?? null,
                  }
                : null,
            ];
          })
    ...
    downloadJson("p2006t-guided-coordinate-map.json", {
      ...
      gridModel: {
        columns: 5,
        rows: 22,
        columnSeed: "two-consecutive-columns-four-clicks",
        rowSeed: "sl-ground-roll,sl-50ft,1000-ground-roll",
        rowSpacing: "alternating-within-pair-and-between-altitudes",
        axisLock: "vertical-columns-horizontal-rows",
        rowExtent: "first-to-last-temperature-column-centre",
        cellLocation: "intersection",
        calculationNotesRectangle: "per-registration-per-weight-page",
      },
      ...
      <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        <div>
          ...
          <h2 className="mt-1 text-2xl font-semibold tracking-tight text-zinc-950">
            Alternating row pattern and calculation area
          </h2>
          <p className="mt-2 max-w-4xl text-sm leading-6 text-zinc-600">
            Three row centres capture the real repeating pattern. The builder alternates
            the two vertical spacings, locks every row horizontally and lets you reserve
            a white rectangle on each performance sheet for the calculation breakdown.
          </p>
        </div>
      ...
      <div className="grid gap-5 2xl:grid-cols-[minmax(0,1.4fr)_380px]">
        <div className="rounded-3xl border border-zinc-200 bg-white p-4">
          ...
          <section className="mb-3 rounded-2xl border-2 border-sky-300 bg-sky-50 p-4">
            ...
            {captureMode ? (
              <p className="mt-2 rounded-xl bg-amber-100 px-3 py-2 text-sm font-semibold text-amber-950">
                ...
                <>
                  ...
                  {derivedGrid ? (
                    <span className="rounded-xl bg-emerald-100 px-3 py-2.5 text-sm font-semibold text-emerald-900">
                      5 columns · 22 rows · alternating spacing
                    </span>
                  ) : null}
                </>
              ) : null}
          ...
          <div className="max-h-[78vh] overflow-auto rounded-2xl border border-zinc-300 bg-zinc-100 p-2">
            ...
            >
              {stage.type === "performance" ? (
                ...
                <PdfPageImage page={stage.page!} onReady={() => setImageReady(true)} />
              )}
