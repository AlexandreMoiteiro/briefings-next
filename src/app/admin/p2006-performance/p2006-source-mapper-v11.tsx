...
function normalizeLegacyCaptures(store: CaptureStore): CaptureStore {
  const normalized: CaptureStore = { ...store };

  for (const [key, capture] of Object.entries(normalized)) {
    if (!key.endsWith(":row-seed") || capture.points.length !== 4) continue;
...
function lockedColumns(points: Point[]): Segment[] {
  ...
  const firstTopY = Math.min(points[0].y, points[1].y);
  const firstBottomY = Math.max(points[0].y, points[1].y);
  const secondTopY = Math.min(points[2].y, points[3].y);
  const secondBottomY = Math.max(points[2].y, points[3].y);
  const firstX = average(points[0].x, points[1].x);
  const secondX = average(points[2].x, points[3].x);
  const topY = average(firstTopY, secondTopY);
  const bottomY = average(firstBottomY, secondBottomY);
...
function rowSeedYValues(points: Point[]) {
  if (points.length >= 4) {
    return [
      average(points[0].y, points[1].y),
      average(points[2].y, points[3].y),
    ].sort((a, b) => a - b);
  }
...
function lockedRows(points: Point[], columns: Segment[]): Segment[] {
  const seedY = rowSeedYValues(points);
  if (seedY.length !== 2 || columns.length !== 5) return [];

  const spacing = seedY[1] - seedY[0];
  const leftX = columns[0].start.x;
  const rightX = columns[columns.length - 1].start.x;

  return Array.from({ length: 22 }, (_, index) => {
    const y = seedY[0] + spacing * index;
    return {
      start: { x: leftX, y },
      end: { x: rightX, y },
    };
...
function deriveGrid(columnCapture?: Capture, rowCapture?: Capture): DerivedGrid | null {
  ...
  if (columns.length !== 5 || rows.length !== 22) return null;
...
function startButtonLabel(step: GuidedStep) {
  if (step.kind === "confirm") return "Confirm corrected grid and continue";
...
function activeInstruction(step: GuidedStep, capture?: Capture) {
  ...
  if (step.kind === "points") {
    ...
    return `Click point ${done + 1} of ${step.requiredPoints ?? 1}. Alignment and line length are corrected automatically.`;
  }
...
function GenericCaptureOverlay({
  capture,
  step,
  current,
  columns,
  ...
  if (role === "regular-column-seed") {
    ...
    const ys = rowSeedYValues(capture.points);
    const leftX = columns[0].start.x;
    const rightX = columns[columns.length - 1].start.x;
    segments = ys.map((y) => ({
      start: { x: leftX, y },
...
export function P2006TSourceMapperV9() {
  ...
  useEffect(() => {
    ...
    try {
      const normalized = normalizeLegacyCaptures(JSON.parse(saved) as CaptureStore);
  ...
  const gridConfirmation =
    stage.type === "performance"
      ...
      : undefined;
  const currentColumns = columnSeed ? lockedColumns(columnSeed.points) : [];
  const derivedGrid =
  ...
  function exportMap() {
    ...
    downloadJson("p2006t-guided-coordinate-map.json", {
      ...
      gridModel: {
        columns: 5,
        rows: 22,
        columnSeed: "two-consecutive-vertical-lines",
        rowSeed: "two-y-positions-only",
      ...
      <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        <div>
          ...
          <h2 className="mt-1 text-2xl font-semibold tracking-tight text-zinc-950">
            Seed, correct automatically and confirm
          </h2>
          <p className="mt-2 max-w-4xl text-sm leading-6 text-zinc-600">
            Columns are forced vertical. Rows use only two Y positions, are forced
            horizontal, and always run exactly from the first to the last temperature
            column.
          </p>
        </div>
