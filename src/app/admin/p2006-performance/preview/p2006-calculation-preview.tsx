...
function WhiteOverlayCard({
  ...
  return (
    <div
      className={`pointer-events-none absolute overflow-hidden rounded-lg border-2 bg-white p-2.5 shadow-sm ${borderClass}`}
      style={{
        left: `${rect.x * 100}%`,
        top: `${rect.y * 100}%`,
    ...
    >
      <p className="font-bold tracking-tight text-zinc-950">{title}</p>
      <div className="mt-1 space-y-0.5 leading-tight text-zinc-800">{children}</div>
    </div>
  );
}

function MassLimitLine({
  capture,
  mass,
}: {
  capture?: Capture;
...
export function P2006TCalculationPreview() {
  ...
  const mbValidation = useMemo(() => {
        ...
        status:
          !fit.monotonic || residualPixels > 4
            ...
            : residualPixels > 2
              ? "warn"
              : "pass",
        detail: `${config.values.length} points · maximum residual ${residualPixels.toFixed(
          1
        )} px · ${fit.monotonic ? "ordered correctly" : "out of order"}.`,
      });
    }

    for (const guide of GUIDE_CONFIG) {
      const points = linePoints(payload.captures[captureKey(guide.id)]);
      const span = points ? Math.abs(points[1].x - points[0].x) : 0;
      validations.push({
        label: `${guide.label} guide`,
        status: !points ? "fail" : span < 0.06 ? "warn" : "pass",
        detail: points
          ? `Captured across ${(span * 100).toFixed(1)}% of the page width.`
          : "Guide line is missing.",
      });
    }
    ...
    if (y16 === null || y23 === null || y31 === null) {
      validations.push({
        label: "MAC envelope",
        status: "fail",
        detail: "One or more MAC reference lines are missing.",
      });
    } else {
      const expected23 = lerp(y16, y31, (23 - 16.5) / (31 - 16.5));
      const residual = Math.abs(y23 - expected23) * 1000;
      validations.push({
        label: "23% MAC reference",
        status: residual > 12 ? "fail" : residual > 6 ? "warn" : "pass",
        detail: `Deviation from the 16.5–31% interpolation is ${residual.toFixed(
          1
        )} px.`,
      });
    }
    ...
    for (const mass of [1180, 1230]) {
      ...
      if (x === null || expected === undefined) {
        validations.push({
          label: `${mass} kg limit`,
          status: "fail",
          detail: "Limit line or flight-mass calibration is missing.",
        });
      } else {
          ...
          status:
            residual > 12 || verticalError > 5
              ...
              : residual > 6 || verticalError > 2
                ? "warn"
                : "pass",
          detail: `Axis difference ${residual.toFixed(
            1
  ...
  const performanceExample = useMemo(() => {
    ...
    const slopeFactor =
      output === "ground-roll"
        ? 1 + (corrections.slopePercentPerOnePercent / 100) * runwaySlope
        : 1;
    ...
    const lowerPoint = {
      x: lerp(
        grid.columnCenters[temp.lower],
        grid.columnCenters[temp.upper],
        temp.ratio
      ),
      y: grid.rowCenters[lowerRow],
    };
    const upperPoint = {
      x: lerp(
        grid.columnCenters[temp.lower],
      ...
      y: grid.rowCenters[upperRow],
    };
    const finalPoint = {
      x: lowerPoint.x,
      y: lerp(lowerPoint.y, upperPoint.y, altitude.ratio),
    };
  ...
  const performanceValidation = useMemo(() => {
    ...
    if (!grid) {
      validations.push({
        label: "Detected table grid",
        status: "fail",
        detail: "No valid 5 × 22 grid is available for this page.",
      });
    } else {
      ...
      validations.push({
        ...
        detail: `Maximum spacing variation is ${(xVariation * 100).toFixed(1)}%.`,
      });
      validations.push({
        label: "Result rows",
        status: yVariation > 0.25 ? "warn" : "pass",
        detail: `Maximum spacing variation is ${(yVariation * 100).toFixed(
          1
        )}%.`,
      });
    }
    validations.push({
      ...
      detail: performanceTable
        ? `${performanceTable.rows.length} result rows were read from the source text.`
        : "The source text could not be parsed.",
    });
    for (const [stepId, label] of [
      ["published-assumptions", "Assumptions area"],
      ["published-corrections", "Corrections area"],
      ["calculation-notes-rectangle", "Calculation area"],
    ] as const) {
      const capture =
      ...
      validations.push({
        ...
        detail: capture?.rect ? "Mapped." : "Not mapped for this aircraft and page.",
      });
    }
    return validations;
  ...
  const selectedWeight =
    selectedSource.weightKg === 1180
      ? applicableMaxMass
      : selectedSource.weightKg;
  const isTakeoff = selectedSource.performanceKind === "takeoff";

  const explanation = useMemo(() => {
    if (!performanceExample) return [];
    const lines: string[] = [];
    const lowerAltitude = ALTITUDES[performanceExample.altitude.lower];
    const upperAltitude = ALTITUDES[performanceExample.altitude.upper];
    const lowerTemp = TEMPERATURES[performanceExample.temp.lower];
    const upperTemp = TEMPERATURES[performanceExample.temp.upper];

    if (lowerAltitude === upperAltitude) {
      lines.push(`Use the ${lowerAltitude} ft row directly.`);
    } else {
      lines.push(
        `${performanceExample.altitude.clamped.toFixed(
          0
        )} ft lies between ${lowerAltitude} ft and ${upperAltitude} ft.`
      );
    }

    if (lowerTemp === upperTemp) {
      lines.push(`Use the ${lowerTemp} °C column directly.`);
    } else {
      lines.push(
        `${performanceExample.temp.clamped.toFixed(
          1
        )} °C lies between ${lowerTemp} °C and ${upperTemp} °C.`
      );
      lines.push(
        `Interpolate across the ${lowerAltitude} ft row to obtain ${performanceExample.lowerAltitudeValue.toFixed(
          1
        )} m.`
      );
      if (lowerAltitude !== upperAltitude) {
        lines.push(
          `Repeat the temperature interpolation on the ${upperAltitude} ft row to obtain ${performanceExample.upperAltitudeValue.toFixed(
            1
          )} m.`
        );
      }
    ...
    if (lowerAltitude !== upperAltitude) {
      lines.push(
        `Interpolate between both altitude results: ${performanceExample.base.toFixed(
          1
        )} m.`
      );
    }
    ...
    if (windKt === 0) {
      lines.push("No wind correction is applied.");
    } else if (windKt > 0) {
      lines.push(
        `Apply the ${windKt.toFixed(
          0
        )} kt headwind correction (${performanceExample.windCorrection.toFixed(
          1
        )} m).`
      );
    } else {
      lines.push(
        `Apply the ${Math.abs(windKt).toFixed(
          0
        )} kt tailwind correction (+${performanceExample.windCorrection.toFixed(
          1
        )} m).`
      );
    }

    if (output === "ground-roll") {
      lines.push(
        pavedRunway
          ? `Apply the paved-runway reduction (${(
              (1 - performanceExample.pavedFactor) *
              100
            ).toFixed(0)}%).`
          : "No paved-runway correction is applied."
      );
      lines.push(
        runwaySlope === 0
          ? "No runway-slope correction is applied."
          : `Apply the runway-slope correction for ${runwaySlope > 0 ? "+" : ""}${runwaySlope.toFixed(
              1
            )}%.`
      );
    }
    ...
    return lines;
  }, [output, pavedRunway, performanceExample, runwaySlope, windKt]);
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-zinc-500">
              {Object.keys(payload.captures).length} mapped items are available. This
            ...
            <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
              <div>
                <h3 className="text-lg font-semibold text-zinc-950">
                  Mass and balance path
                </h3>
                <p className="text-sm text-zinc-500">
                  Blue segments add each load; dashed segments transfer the result to
                  the next panel.
                </p>
              </div>
                ...
                <>
                  ...
                  >
                    ...
                    <MassLimitLine
                      capture={
                        payload.captures[
                          captureKey(`mass-limit-${applicableMaxMass}`)
                        ]
                      }
                      mass={applicableMaxMass}
                    />
                    ...
                    {mbExample?.segments.map((segment, index) => (
                      <line
                        ...
                        strokeWidth={segment.kind === "load" ? 3.5 : 2}
                        strokeDasharray={
                  ...
                  {mbExample ? (
                    <div className="pointer-events-none absolute bottom-3 left-3 w-[min(260px,44%)] rounded-xl border border-zinc-300 bg-white p-3 shadow-md">
                      <p className="text-sm font-bold text-zinc-950">
                        Mass and balance
                      </p>
                      <div className="mt-2 space-y-1 text-xs leading-5 text-zinc-800">
                        <p>
                          Total mass: <strong>{mbExample.totalMass.toFixed(0)} kg</strong>
                        </p>
                        <p>
                          Final moment: <strong>{mbExample.totalMoment.toFixed(1)} kg·m</strong>
                        </p>
                        <p>
                          Estimated CG: <strong>{mbExample.mac?.toFixed(1) ?? "—"}% MAC</strong>
                        </p>
                        <p>Applicable limit: <strong>{mbExample.maxMass} kg</strong></p>
                        <p
                          className={
                            mbExample.withinCg && mbExample.withinMass
                        ...
                        >
                          {mbExample.withinCg && mbExample.withinMass
                            ...
                            : "Outside the applicable envelope"}
                        </p>
                      </div>
                    </div>
                  ) : null}
            ...
            <section className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm">
              ...
              {mbExample ? (
                <div className="mt-4 rounded-2xl border border-zinc-200 bg-zinc-50 p-4 text-sm text-zinc-800">
                  <p className="font-semibold text-zinc-950">Calculated position</p>
                  <p className="mt-2">
                    The loading sequence finishes at {mbExample.totalMass.toFixed(0)} kg
                    and {mbExample.totalMoment.toFixed(1)} kg·m.
                  </p>
                  <p className="mt-1">
                    The estimated CG is {mbExample.mac?.toFixed(1) ?? "—"}% MAC, with a
                    maximum permitted mass of {mbExample.maxMass} kg for {registration}.
                  </p>
                </div>
              ) : (
                <p className="mt-4 rounded-xl bg-red-50 p-3 text-sm font-semibold text-red-800">
                  Complete the six axis calibrations and four loading guides before
                  calculating the path.
                </p>
              )}
            </section>

            <ValidationList title="Geometry checks" validations={mbValidation} />
          </aside>
        </div>
      ) : (
        <div className="grid gap-5 2xl:grid-cols-[minmax(0,1.5fr)_430px]">
          <section className="rounded-3xl border border-zinc-200 bg-white p-4 shadow-sm">
            <div className="mb-3 flex flex-wrap items-end justify-between gap-3">
              <div>
                ...
                <p className="text-sm text-zinc-500">
                  The coloured cells show the exact values used for temperature and
            ...
            <div className="relative overflow-hidden rounded-2xl border border-zinc-300 bg-white">
              ...
              >
                ...
                {grid && performanceExample
                  ? performanceExample.selected.map((cell, index) => {
                      const lowerTone = cell.row === performanceExample.lowerRow;
                      return (
                        <g key={`${cell.row}-${cell.column}`}>
                          <rect
                            ...
                            stroke={
                              lowerTone ? "rgb(217 119 6)" : "rgb(37 99 235)"
                            }
                            strokeWidth="2"
                            vectorEffect="non-scaling-stroke"
                          />
                          <circle
                            cx={grid.columnBounds[cell.column] * 1000 + 12}
                            cy={grid.rowBounds[cell.row] * 1000 + 12}
                            r="9"
                            fill="white"
                            stroke={
                              lowerTone ? "rgb(217 119 6)" : "rgb(37 99 235)"
                          ...
                          />
                          <text
                            x={grid.columnBounds[cell.column] * 1000 + 12}
                            y={grid.rowBounds[cell.row] * 1000 + 16}
                            textAnchor="middle"
                            fontSize="10"
                            fontWeight="800"
                            fill={
                              lowerTone ? "rgb(180 83 9)" : "rgb(29 78 216)"
                            }
                          >
                            {index + 1}
                          </text>
                ...
                {grid && performanceExample ? (
                  <>
                    {performanceExample.temp.lower !== performanceExample.temp.upper ? (
                      <>
                        <line
                          x1={grid.columnCenters[performanceExample.temp.lower] * 1000}
                          y1={grid.rowCenters[performanceExample.lowerRow] * 1000}
                          x2={grid.columnCenters[performanceExample.temp.upper] * 1000}
                          y2={grid.rowCenters[performanceExample.lowerRow] * 1000}
                          ...
                          vectorEffect="non-scaling-stroke"
                        />
                        {performanceExample.upperRow !== performanceExample.lowerRow ? (
                          <line
                            x1={grid.columnCenters[performanceExample.temp.lower] * 1000}
                            y1={grid.rowCenters[performanceExample.upperRow] * 1000}
                            x2={grid.columnCenters[performanceExample.temp.upper] * 1000}
                            y2={grid.rowCenters[performanceExample.upperRow] * 1000}
                    ...
                    {performanceExample.upperRow !== performanceExample.lowerRow ? (
                      <line
                        x1={performanceExample.lowerPoint.x * 1000}
                        y1={performanceExample.lowerPoint.y * 1000}
                        x2={performanceExample.upperPoint.x * 1000}
                        y2={performanceExample.upperPoint.y * 1000}
                        stroke="rgb(51 65 85)"
                        strokeWidth="1.8"
                        strokeDasharray="7 5"
                        strokeLinecap="round"
                    ...
                    <circle
                      cx={performanceExample.finalPoint.x * 1000}
                      cy={performanceExample.finalPoint.y * 1000}
                      r="6"
                      fill="rgb(22 163 74)"
                      stroke="white"
                      strokeWidth="2.5"
              ...
              >
                <p>Weight: {selectedWeight} kg</p>
                <p>Flaps: {isTakeoff ? "T/O" : "LAND"}</p>
                <p>{isTakeoff ? "Lift-off speed: 65 KIAS" : "Approach speed: 70 KIAS"}</p>
                <p>{isTakeoff ? "50 ft speed: 70 KIAS" : "Throttle levers: idle"}</p>
                <p>Published surface: grass</p>
              </WhiteOverlayCard>
              ...
              >
                <p>Headwind: −{performanceTable?.corrections.headwindMetersPerKt ?? 0} m per kt</p>
                <p>Tailwind: +{performanceTable?.corrections.tailwindMetersPerKt ?? 0} m per kt</p>
                <p>Paved runway: −{performanceTable?.corrections.pavedPercent ?? 0}%</p>
                <p>
                  Slope: {performanceTable?.corrections.slopePercentPerOnePercent ?? 0}% per 1%
                </p>
              </WhiteOverlayCard>

              <WhiteOverlayCard
                rect={calculationCapture?.rect}
                title="Calculation"
                borderClass="border-zinc-900"
              >
                {explanation.map((line, index) => (
                  <p key={`explanation-${index}`}>{line}</p>
                ))}
                {performanceExample ? (
                  <p className="pt-1 font-bold text-zinc-950">
                    Final result: {performanceExample.final.toFixed(0)} m
                  </p>
                ) : null}
              </WhiteOverlayCard>
            </div>
          ...
          <aside className="space-y-4">
            <section className="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm">
              ...
              {performanceExample ? (
                <div className="mt-4 rounded-2xl border border-zinc-200 bg-zinc-50 p-4 text-sm leading-6 text-zinc-800">
                  <p className="font-semibold text-zinc-950">Calculation summary</p>
                  <p className="mt-2">
                    The table gives {performanceExample.base.toFixed(1)} m after
                    temperature and altitude interpolation.
                  </p>
                  <p>
                    After the published corrections, the result is{" "}
                    <strong>{performanceExample.final.toFixed(0)} m</strong>.
                  </p>
                </div>
              ) : null}
            </section>
