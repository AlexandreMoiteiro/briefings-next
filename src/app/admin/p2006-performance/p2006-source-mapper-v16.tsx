...
const MIGRATION_KEY = "briefings_p2006_manual_rows_v16";
const MIN_ZOOM = 50;
const MAX_ZOOM = 300;
const ZOOM_STEP = 25;
...
function isManualRow(step?: GuidedStep) {
  return Boolean(step && roleOf(step) === "manual-row-centre");
}

function rowIndexOf(step: GuidedStep) {
  const value = Number(step.metadata?.rowIndex ?? -1);
...
function migrateCaptures(input: CaptureStore): CaptureStore {
  ...
  for (const key of Object.keys(next)) {
    ...
    if (key.endsWith(":row-seed") || key.endsWith(":grid-confirmation")) {
      delete next[key];
    }
  }
...
function orderedRowSteps(stage: Stage) {
  return stage.steps
    .filter(isManualRow)
    .sort((a, b) => rowIndexOf(a) - rowIndexOf(b));
}

function capturedRowYs(
  stage: Stage,
  registration: P2006TRegistration,
  captures: CaptureStore
) {
  return orderedRowSteps(stage).map((rowStep) => {
    const capture = captures[mappingKey(stage, registration, rowStep)];
    return capture?.points[0]?.y;
  });
}
...
function deriveGrid(
  ...
  const rowYs = capturedRowYs(stage, registration, captures);
  if (xs.length !== 5 || rowYs.length !== 22) return null;
  if (rowYs.some((value) => typeof value !== "number")) return null;

  const ys = rowYs as number[];
  const columns = xs.map((x) => ({
    start: { x, y: Math.min(...ys) },
    end: { x, y: Math.max(...ys) },
  }));
  const rows = ys.map((y) => ({
    start: { x: xs[0], y },
...
function startButtonLabel(step: GuidedStep) {
  if (step.kind === "confirm") return "Confirm complete grid and continue";
  if (step.kind === "rect") return "Start — drag the rectangle";
  if (isManualRow(step)) return "Start — mark this row";
...
function activeInstruction(step: GuidedStep, capture?: Capture) {
  if (step.kind === "rect") {
    return roleOf(step) === "calculation-notes-rectangle"
      ? "Drag the white calculation area. Release to save it and continue."
      : "Drag from one corner to the opposite corner.";
  }
  if (isManualRow(step)) {
    return "Click once on the centre of this row. The X position is ignored and the next row opens automatically.";
  }
...
function CaptureOverlay({
  ...
  current: boolean;
  xs: number[];
  rowRange: { top: number; bottom: number } | null;
}) {
  ...
  if (role === "regular-column-seed" && capture.points.length >= 4) {
    ...
    const top = rowRange?.top ?? Math.min(...capture.points.map((point) => point.y));
    const bottom =
      rowRange?.bottom ?? Math.max(...capture.points.map((point) => point.y));
    ownXs.forEach((x) =>
      segments.push({ start: { x, y: top }, end: { x, y: bottom } })
    );
  }

  if (role === "manual-row-centre" && capture.points[0] && xs.length === 5) {
    const y = capture.points[0].y;
    segments.push({
      start: { x: xs[0], y },
      end: { x: xs[xs.length - 1], y },
    });
...
export function P2006TSourceMapper() {
  ...
  const rowSteps = orderedRowSteps(stage);
  ...
  const rowValues = capturedRowYs(stage, registration, captures).filter(
    (value): value is number => typeof value === "number"
  );
  const rowRange = rowValues.length
    ? { top: Math.min(...rowValues), bottom: Math.max(...rowValues) }
    : null;
  const derivedGrid =
    stage.type === "performance"
      ? deriveGrid(stage, registration, captures, columnCapture)
      : null;
  ...
  const capturedRows = rowSteps.filter(
    (rowStep) => captures[mappingKey(stage, registration, rowStep)]?.confirmed
  ).length;
  ...
  function saveAndAdvance(capture: Capture) {
    ...
    const nextStep = stage.steps[nextIndex];

    if (nextStep && isManualRow(nextStep)) {
  ...
  function redoColumns() {
    ...
    const targetIndex = stage.steps.indexOf(columnStep);
  ...
  function redoAllRows() {
    if (!rowSteps.length) return;
    const firstRowIndex = stage.steps.indexOf(rowSteps[0]);
    const confirmationKey = confirmationStep
      ? mappingKey(stage, registration, confirmationStep)
      : null;
    ...
    setCaptures((current) => {
      ...
      rowSteps.forEach((rowStep) => {
        delete next[mappingKey(stage, registration, rowStep)];
      });
      if (confirmationKey) delete next[confirmationKey];
      return next;
    });
    setStepIndex(firstRowIndex);
    setCaptureMode(true);
    setDrag(null);
  }
  ...
  function exportMap() {
    ...
    downloadJson("p2006t-guided-coordinate-map.json", {
      ...
      gridModel: {
        ...
        rows: "22-individual-row-centres",
        rowCount: 22,
  ...
  const rowSequenceActive = stage.type === "performance" && isManualRow(step);
  const visibleCaptures = stageCaptures.filter((entry) => {
    if (!entry.capture || entry.capture.kind === "confirm") return false;
    if (entry.key === currentKey || showCompleted) return true;
    if (rowSequenceActive && isManualRow(entry.step)) return true;
    if (rowSequenceActive && entry.step.id === "column-seed") return true;
    return false;
  });
      ...
      <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        <div>
          ...
          <h2 className="mt-1 text-2xl font-semibold tracking-tight text-zinc-950">
            Columns once, then every row individually
          </h2>
          <p className="mt-2 max-w-4xl text-sm leading-6 text-zinc-600">
            The columns remain automatic. Each of the 22 Ground Roll/50 ft rows now has
            its own one-click task, so there is no estimated spacing or cumulative drift.
          </p>
        </div>
          ...
          <section className="mb-3 rounded-2xl border-2 border-sky-300 bg-sky-50 p-4">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div className="max-w-3xl">
                ...
                {rowSequenceActive ? (
                  <p className="mt-2 rounded-xl bg-emerald-100 px-3 py-2 text-sm font-semibold text-emerald-900">
                    Rows: {capturedRows}/22 marked. Keep clicking; the next row opens automatically.
                  </p>
                ) : null}
                {captureMode ? (
                  ...
                  <>
                    ...
                    {derivedGrid ? (
                      <span className="rounded-xl bg-emerald-100 px-3 py-2.5 text-sm font-semibold text-emerald-900">
                        5 columns · 22 manual rows · 110 intersections
                      </span>
                    ) : null}
                  </>
                ) : null}
