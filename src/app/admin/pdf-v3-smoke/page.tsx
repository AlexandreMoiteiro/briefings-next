...
import { buildP2006TPerformancePdfV3 } from "@/lib/pdf/p2006t-performance-pdf-v3";
...
export default async function PdfV3SmokePage() {
  ...
  try {
    ...
    const conditions = {
      weightKg: 1150,
      isaDeviationC: 0,
      cruiseRpm: 2100 as const,
      cruisePowerPercent: 65,
    };
    const climb = p2006tClimbPerformance("D-GSEV", 1500, conditions);
    const cruise = p2006tCruisePerformance("D-GSEV", 3000, conditions);
    ...
    if (rows.length !== 4) throw new Error("Four performance rows were expected.");
    ...
    console.log("PDF_V3_SMOKE_OK", JSON.stringify(result));
