...
import {
  getP2006TTaxiMinutes,
  resetP2006TTaxiMinutes,
  setP2006TTaxiMinutes,
} from "@/lib/performance/p2006t-taxi-time-store";
...
function calculatedTimes(section: HTMLElement, sources: SourceMap) {
  const taxi = getP2006TTaxiMinutes();
...
function makeTimeInput(key: string, source: HTMLInputElement | null) {
  ...
  input.value = formatOperationalMinutes(
    key === "taxi" ? getP2006TTaxiMinutes() : numberValue(source)
  );

  const commit = () => {
    const current = key === "taxi" ? getP2006TTaxiMinutes() : numberValue(source);
    const minutes = parseMinutes(input.value);
    if (minutes === null) {
      input.value = formatOperationalMinutes(current);
      return;
    }
    if (key === "taxi") {
      setP2006TTaxiMinutes(minutes);
    } else {
...
function buildFuelTable(section: HTMLElement, sources: SourceMap) {
  ...
  note.textContent =
    "Start-up and Taxi time is editable independently from its fuel allowance. Times accept minutes, HH:MM or values such as 1 h 40 min; fuel is shown as a whole-litre approximation.";
...
function refreshFuelTable(section: HTMLElement, table: HTMLElement, sources: SourceMap) {
  ...
  table.querySelectorAll<HTMLInputElement>("input[data-time-source]").forEach((input) => {
    ...
    const key = input.dataset.timeSource ?? "";
    const next = formatOperationalMinutes(
      key === "taxi" ? getP2006TTaxiMinutes() : numberValue(sources[key])
    );
...
function enhancePdfContents(root: HTMLElement) {
  ...
  const optionCards = Array.from(
    content.querySelectorAll('label:has(input[type="checkbox"])')
  ) as HTMLLabelElement[];
  ...
  if (!summary) {
    ...
    summary.className =
      "mt-4 rounded-xl bg-zinc-100 px-4 py-3 text-sm font-medium text-zinc-700";
...
export function P2006TMissionClientV8() {
