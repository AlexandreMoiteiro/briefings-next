...
AIRCRAFT_PROFILES = {
    ...
    "Piper PA-28": {
        "climb_tas":   76.0,
        "cruise_tas":  110.0,   # alterado de 115 para 110
        "descent_tas": 100.0,
...
if st.session_state.wps:
    ...
    for i, w in enumerate(st.session_state.wps):
        with st.expander(f"WP {i+1} — {w['name']} ({w['id']})", expanded=False):
            ...
            # STOP: máximo alargado para 480 min (8 horas), step 1 min
            stop_min = st.number_input(
                ...
                step=1.0,
                key=f"wpstop_{w['id']}",
                help="Tempo de espera/circuito neste ponto. Sem limite prático — podes colocar várias horas."
            )
